...
<div id="con_wrap">
	<div id="tab">
		...
		<ul>
		    <li><a href="<c:url value='/memadmin'/>" style="margin-left:0px;" class="hover_a">전체회원</a></li>
		...
		<div style="position:absolute;top:540px;width: 100%;"align="center">
			<c:choose>
				<c:when test="${pu.startPageNum>5 }">
					<a href="<c:url value='/memadmin?pageNum=${pu.startPageNum-1 }&text=${text }'/>">
			...
			<c:forEach var="i" begin="${pu.startPageNum }" end="${pu.endPageNum }">
				<c:choose>
					<c:when test="${i==pu.pageNum }">
						<a href="<c:url value='/memadmin?pageNum=${i }&text=${text }'/>">
			...
			<c:choose>
				<c:when test="${pu.endPageNum<pu.totalPageCount}">
					<a href="<c:url value='//memadmin?pageNum=${pu.endPageNum+1 }&text=${text }'/>">
						<span class="btnnext">
								▷▷
						</span>
					</a>
