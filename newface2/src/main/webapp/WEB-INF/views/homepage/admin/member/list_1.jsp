	...
	$(document).ready(function() {
		...
		$("#search").click(function(){
			...
			if(text==""){
				...
				location.href="<c:url value='/memadmin/1'/>?text="+text;
			}
		});
		...
		<div style="position:absolute;top:540px;width: 100%;"align="center">
			<c:choose>
				<c:when test="${pu.startPageNum>5 }">
					<a href="<c:url value='/memadmin/1?pageNum=${pu.startPageNum-1 }&text=${text }'/>">
			...
			<c:forEach var="i" begin="${pu.startPageNum }" end="${pu.endPageNum }">
				<c:choose>
					<c:when test="${i==pu.pageNum }">
						<a href="<c:url value='/memadmin/1?pageNum=${i }&text=${text }'/>">
